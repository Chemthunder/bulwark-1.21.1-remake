package net.kindling.bulwark.impl.client.particle;
...
public class SculkShockwaveParticle extends SpriteBillboardParticle {
    private final SpriteProvider sprites;

    private static final Quaternionf QUATERNION = new Quaternionf(0.0F, -0.7F, 0.7F, 0.0F);

    public SculkShockwaveParticle(ClientWorld world, double x, double y, double z, double velX, double velY, double velZ, SpriteProvider sprites) {
    ...
    @Override
    protected void method_60373(VertexConsumer buffer, Camera camera, Quaternionf quaternionf, float ticks) {
    ...
    public record Factory<T extends ParticleEffect>(SpriteProvider sprites) implements ParticleFactory<T> {
        public Particle createParticle(T type, ClientWorld world, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new SculkShockwaveParticle(world, x, y, z, xSpeed, ySpeed, zSpeed, sprites);
        }
    }
}